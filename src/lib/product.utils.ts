...
export const truncateText = (
  ...
  if (!text) return ""; // Handles undefined, null, empty
...
export const formatDiscountDisplay = (
  ...
  if (discountType === "2")
    return `₦${parseFloat(discountValue).toLocaleString()}`;
  return "Unknown";
};
...
export const shouldShowStrikethrough = (product: Product): boolean => {
  return (
    product.discountType !== "0" && product.unitPrice !== product.discountPrice
  );
};
